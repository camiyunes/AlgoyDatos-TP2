...
public class Jugador {
	...
	public Jugador(String nombre) throws Exception {
		...
		for (int i = 0; i < this.mano.length; i++) { this.mano[i] = null; }
    }

	public Jugador(String nombre, int cartas) throws Exception {
		validarNombre(nombre);
		if (cartas < 1) { throw new Exception("El jugador debe poder agarrar cartas"); }
		this.nombre = nombre;
	...
	public void perderTurno() { this.turnoActivo = false; } // El jugador pierde su turno

	public void reiniciarTurno() { this.turnoActivo = true; } // Reiniciar el turno al comenzar de nuevo
	...
	public void robarDelMazo(Mazo mazo) throws Exception {
		validarMazo(mazo);
		if (this.getCapacidadRestante() <= 0) { throw new Exception("Tu mano está llena. Probá jugar una carta"); }
	...
	public boolean tieneTodasLasFichasEnElTablero() { return this.tieneTodasLasFichasEnElTablero; }
}